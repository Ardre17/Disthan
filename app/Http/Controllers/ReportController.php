        ...
        public function imprimirMovimientos(Request $request)
    {
        ...
        if (!$tipo || $tipo === 'MATERIA_PRIMA') {
            ...
            foreach ($entradas as $entrada) {
                ...
                $movimientos->push([
                    ...
                    'fecha' => $entrada->created_at,
                    'titulo' => 'Ingreso de materia prima',
                    'referencia' => $entrada->material_codigo ?? '—',
                    'descripcion' => $entrada->material_nombre ?? 'Materia prima',
                    'cantidad' => (float) $entrada->quantity,
                    'unidad' => '—',
                    'usuario' => $entrada->usuario ?? '—',
                    'tercero' => $entrada->supplier ?? '—',
                    'observacion' => $entrada->observation ?? '—',
                ]);
        ...
        */
        ...
        if (!$tipo || $tipo === 'PRODUCCION') {
            ...
            $producciones = $query
                ->orderByDesc('production_orders.fecha_produccion')
                ->get();
            ...
            foreach ($producciones as $produccion) {
                ...
                $movimientos->push([
                    ...
                    'titulo' => 'Producción terminada',
                    'referencia' => $produccion->number,
                    'descripcion' => $produccion->producto_nombre ?? 'Producto terminado',
                    'cantidad' => (float) $produccion->produced_quantity,
                    'unidad' => '—',
                    'usuario' => $produccion->usuario ?? '—',
                    'tercero' => $produccion->material_nombre ?? '—',
                    'observacion' => $produccion->observation ?? '—',
                ]);
        ...
        */
        ...
        if (!$tipo || $tipo === 'PEDIDO') {
            ...
            foreach ($pedidos as $pedido) {
                ...
                $movimientos->push([
                    'tipo' => 'PEDIDO',
                    'fecha' => $pedido->fecha_cierre,
                    'titulo' => 'Pedido cerrado',
                    'referencia' => $pedido->numero_orden,
                    'descripcion' => $pedido->cliente_nombre ?? 'Cliente no registrado',
                    'cantidad' => null,
                    'unidad' => null,
                    'usuario' => '—',
                    'tercero' => $pedido->tipo_orden ?? '—',
