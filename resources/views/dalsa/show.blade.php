...
<div class="erp-bar">
    <div class="erp-bar-left">
        ...
        <div class="erp-module">Kardex Dalsa — {{ $dalsa->nombre }}</div>
    </div>
    <span style="font-size:11px;color:#5a8abf;">Logística › Dalsa › Kardex</span>
</div>
...
@php
    ...
    $totalEntradas = $dalsa->movements->where('tipo','ENTRADA')->sum('cantidad');
    $totalSalidas  = $dalsa->movements->where('tipo','SALIDA')->sum('cantidad');
    $despachado    = $dalsa->cantidad_actual <= 0;
@endphp

<div class="page-hdr">
    <div>
        <div class="page-title">{{ $dalsa->nombre }}</div>
        <div style="font-size:11px;color:#64748b;margin-top:2px;">
            Proveedor: <strong>{{ $dalsa->proveedor }}</strong>
            · Kardex individual · historial de movimientos
        </div>
    </div>
    <a href="{{ route('dalsa.index') }}" class="btn-back">← Volver al almacén</a>
</div>
        ...
        <div class="card-body">
            <div class="stock-hero" style="background:{{ $stBg }};">
                ...
                <div class="stock-big" style="color:{{ $stkC }};">
                    {{ number_format($dalsa->cantidad_actual, 0) }}
                </div>
            ...
            <div class="info-row">
                <span>Proveedor</span>
                <span class="info-val">{{ $dalsa->proveedor }}</span>
            </div>
            <div class="info-row">
                <span>Origen</span>
                <span>
                    <span class="meta-tag tag-origen">🗺️ {{ $dalsa->origen }}</span>
                </span>
            </div>
            <div class="info-row">
                <span>Fecha de llegada</span>
                <span class="info-val">
                    {{ \Carbon\Carbon::parse($dalsa->fecha_llegada)->format('d M Y') }}
                </span>
            </div>
            <div class="info-row">
...
<div>
    <div class="card" style="position:sticky;top:10px;">
        ...
        <div class="card-body">
            ...
            <form method="POST"
                  action="{{ route('dalsa.movimiento', $dalsa) }}">
            @csrf
