...
:root{
    ...
    --erp-accent:#0b5ed7;--erp-danger:#c0312b;--erp-danger-bg:#fbe9e8;
...
.color-dot{width:14px;height:14px;border-radius:50%;display:inline-block;border:2px solid rgba(0,0,0,.1);flex-shrink:0;}
.color-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;padding:3px 10px;border-radius:3px;background:#f1f5f9;color:#1c2733;}
...
.tipo-btn{padding:10px;border:2px solid var(--erp-border);border-radius:3px;font-size:13px;font-weight:700;cursor:pointer;text-align:center;background:#fff;color:var(--erp-ink-muted);transition:.15s;}
...
.btn-edit-top:hover{background:#fef3c7;}
</style>
...
<div class="erp-bar">
    <div class="erp-bar-left">
        ...
        <div class="erp-module">Kardex — {{ $precinto->nombre }}</div>
    </div>
    <span style="font-size:11px;color:#5a8abf;">Inventario › Precintos › Kardex</span>
</div>
...
@php
    ...
    $colorDot = match($precinto->color){ 'VERDE'=>'#22c55e','BLANCO'=>'#e2e8f0','NEGRO'=>'#1e293b',default=>'#94a3b8' };
    $colorLbl = match($precinto->color){ 'VERDE'=>'🟢 Verde','BLANCO'=>'⚪ Blanco','NEGRO'=>'⚫ Negro',default=>$precinto->color };
    $totalEntradas = $precinto->movements->where('tipo','ENTRADA')->sum('cantidad');
    $totalSalidas  = $precinto->movements->where('tipo','SALIDA')->sum('cantidad');
@endphp

<div class="page-hdr">
    <div>
        <div class="page-title">{{ $precinto->nombre }}</div>
        <div style="font-size:11px;color:#64748b;margin-top:2px;">
            Kardex individual · historial completo de movimientos
        </div>
    </div>
    <div style="display:flex;gap:7px;flex-wrap:wrap;">
        <a href="{{ route('precintos.edit', $precinto) }}" class="btn-edit-top">✏️ Editar</a>
        <a href="{{ route('precintos.index') }}" class="btn-back">← Volver</a>
    </div>
</div>

<div class="layout">

{{-- ── Columna izquierda: info + kardex ── --}}
<div>

    {{-- Info general --}}
    <div class="card">
        <div class="card-hdr">Información del precinto</div>
        <div class="card-body">
            ...
            <div class="info-row">
                ...
                <span class="info-val" style="color:var(--erp-warn);">
                    {{ number_format($precinto->stock_minimo, 0) }} unidades
                </span>
            </div>
            <div class="info-row">
            ...
            <div class="info-row">
                ...
                <span class="info-val" style="font-size:11px;">
                    {{ $precinto->updated_at->format('d M Y H:i') }}
                </span>
            </div>
        </div>
    </div>
...
{{-- ── Columna derecha: formulario movimiento ── --}}
<div>
    <div class="card" style="position:sticky;top:10px;">
        ...
        <div class="card-body">
            ...
            <form method="POST"
                  action="{{ route('precintos.movimiento', $precinto) }}"
                  id="formMov">
            @csrf
            ...
            <div class="field">
                <label class="flabel">
                    Cantidad <span style="color:#c0312b;">*</span>
                </label>
                <input
                    type="number"
                    name="cantidad"
            ...
            <div style="background:#f4f6f9;border-radius:3px;padding:12px;text-align:center;">
                ...
                <div style="font-size:10px;color:#94a3b8;margin-top:2px;">
                    precintos · mín. {{ number_format($precinto->stock_minimo, 0) }}
                </div>
                <div style="margin-top:8px;">
                    <span class="status-badge {{ $sbCls }}">{{ $sbLbl }}</span>
                </div>
            ...
            @if($est === 'STOCK_BAJO' || $est === 'AGOTADO')
            <div style="
                background:{{ $est === 'AGOTADO' ? 'var(--erp-danger-bg)' : 'var(--erp-warn-bg)' }};
                border:1px solid {{ $est === 'AGOTADO' ? '#f3c7c4' : '#f9d5a3' }};
                border-radius:3px;padding:8px 10px;margin-top:8px;
                font-size:11px;font-weight:600;
                color:{{ $est === 'AGOTADO' ? 'var(--erp-danger)' : 'var(--erp-warn)' }};
                display:flex;align-items:center;gap:6px;
            ">
                {{ $est === 'AGOTADO' ? '🔴 Sin stock — reponer urgente' : '⚠️ Stock bajo — considerar reposición' }}
            </div>
            @endif
